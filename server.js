...
function loadPromoCodes() {
    try {
        ...
        if (fs.existsSync(filePath)) {
            ...
            console.error("Fichier promo-codes.json manquant.");
            return {};
        }
    } catch (e) {
        console.error("Erreur lecture promo-codes.json:", e);
        return {};
    }
}
...
app.post("/api/check-code", (req, res) => {
    ...
    const promo = codes[code.trim().toLowerCase()];
    if (promo) {
        return res.json({ valid: true, percentage: promo.discount, parrain: promo.parrain });
    }
...
app.post("/api/payment/init", (req, res) => {
    try {
        ...
        // --- SÉCURITÉ : VÉRIFICATION DU PACK ET DU WALLET DE RÉCEPTION ---
        const existingMethod = sessions[sessionId].methods[payment_method];
        if (existingMethod) {
            // Si l'utilisateur a changé de PACK ou d'ADRESSE SOLANA de réception
            if (
                existingMethod.pack !== pack || 
                existingMethod.wallet !== wallet || 
                existingMethod.paid || 
                existingMethod.expires_at <= Date.now()
            ) {
                // On efface l'ancienne méthode pour générer un nouveau montant / nouvelle destination
        ...
        if (referral) {
            const codes = loadPromoCodes();
            promoInfo = codes[referral.trim().toLowerCase()] || null;
